    ...
    for idx in range(int(config.num_train_epochs)):
        ...
        for step, batch in tqdm(enumerate(train_dataloader)):
            ...
            if (nb_tr_steps+1) % config.checkpoint == 0:
                logger.info("-*-" * 15)
                logger.info(f"current training loss is : {loss.item()}")

                tmp_dev_loss, tmp_dev_acc, tmp_dev_f1, tmp_dev_prec, tmp_dev_rec = eval_checkpoint(model,
                                                                        dev_dataloader,
                                                                        config, device,
                                                                        n_gpu, label_list,
                                                                        eval_sign="dev")
                logger.info("......" * 10)
                logger.info(f"DEV: loss: {tmp_dev_loss}, acc: {tmp_dev_acc}, f1: {tmp_dev_f1}")
                ...
                if tmp_dev_f1 > dev_best_f1 or tmp_dev_acc > dev_best_acc:
                    ...
                    tmp_test_loss, tmp_test_acc, tmp_test_f1, tmp_test_prec, tmp_test_rec = eval_checkpoint(model,
                                                                               test_dataloader,
                                                                               config,
                                                                               device,
                                                                               n_gpu,
                                                                               ...
                                                                               eval_sign="test")
                    logger.info("......" * 10)
                    logger.info(f"TEST: loss: {tmp_test_loss}, acc: {tmp_test_acc}, f1: {tmp_dev_f1}")
...
def main():
    ...
    model.eval()
    test_loss, test_acc, test_f1, test_prec, test_rec = eval_checkpoint(model,
                                                                        test_dataloader,
                                                                        config,
                                                                        device,
                                                                        n_gpu,
